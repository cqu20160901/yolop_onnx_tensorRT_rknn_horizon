...
ONNX_MODEL = './yolop_640x384.onnx'
RKNN_MODEL = './yolop_640x384.rknn'
DATASET = './dataset.txt'
...
def export_rknn_inference(img):
    ...
    # pre-process config
    print('--> Config model')
    rknn.config(mean_values=[[123, 117, 104]], std_values=[[58, 57, 57]], quantized_algorithm='normal', quantized_method='channel', target_platform='rk3588')
    print('done')

    # Load ONNX model
    print('--> Loading model')
    ret = rknn.load_onnx(model=ONNX_MODEL, outputs=['output1', 'output2', 'output3', 'output4', 'output5'])
    if ret != 0:
...
if __name__ == '__main__':
    ...
    img_path = './test.jpg'
    origimg1 = cv2.imread(img_path)
    img_h, img_w = origimg1.shape[:2]
    
    origimg = cv2.resize(origimg1, (input_imgW, input_imgH))
    ...
    cv2.imwrite('./rknn_result.jpg', origimg1)
